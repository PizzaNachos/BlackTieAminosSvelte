<script lang="ts">
	...
	import type { filter_definition } from '$lib/interfaces';
	...
	$: if (data && data?.product?.multi == true) {
		...
		me = data.product.items[my_index];
	} else {
		me = data.product;
		multi = false;
		items = undefined;
	...
	$: my_rating = Math.ceil(data.reviews.reduce((a, b) => a + b.rating, 0) / my_nums_reviews);
